...
Modular orchestrator that delegates to specialized handlers:
...
class MusicCleanupOrchestrator:
    """
    Main orchestrator for DJ music cleanup.
    ...
    def organizer(self):
        ...
        if self._organizer is None:
            ...
            # Get organization structure from config
            structure = "genre/decade"  # Default to genre/decade as requested
            if 'organization' in self.config and 'structure' in self.config['organization']:
                structure = self.config['organization']['structure']
            
            # Get naming pattern from config
            naming_pattern = "{year} - {artist} - {title} [QS{score}%]"  # Default pattern with year
            if 'organization' in self.config and 'naming_pattern' in self.config['organization']:
                naming_pattern = self.config['organization']['naming_pattern']
    ...
    def _analyze_file_complete(self, file_path: str) -> Optional[Dict[str, Any]]:
        """
        Analyze a single file using the unified FileAnalyzer.
        Returns file info dict for batch duplicate detection.
        
        This method is maintained for backward compatibility but delegates
        to the FileAnalyzer for actual analysis.
        """
        try:
            # Use FileAnalyzer for unified analysis
            ...
            # Convert FileAnalysisResult to legacy file_info format
            file_info = {
                ...
                'algorithm': result.fingerprint_algorithm
            }
            
            # Add defects information if available
